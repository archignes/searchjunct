...
export const FaviconImage: React.FC<{ system: System, focus_mode?: boolean, mini_mode?: boolean, className?: string }> = ({ system, focus_mode, mini_mode, className }) => {  

  if (system.favicon?.startsWith('http')) {
    return (
      <img src={system.favicon}
      alt={`${system.name} favicon`} width={15} height={15}
        className={`rounded-md p-1 ${focus_mode ? 'flex-shrink-0 w-8 h-8 mr-2' : mini_mode ? 'bg-white w-4 h-4 mx-1' : 'flex-shrink-0 w-6 h-6 mr-2'} ${className}`} />
    )
  } else {
    return (
      <Image
        ...
        className={`rounded-md p-1 ${focus_mode ? 'flex-shrink-0 w-8 h-8 mr-2' : mini_mode ? 'bg-white w-4 h-4 mx-1' : 'flex-shrink-0 w-6 h-6 mr-2'} ${system.addFaviconBackground === 'black' ? 'bg-black' : ''} ${className}`} />
    )} 
  }
